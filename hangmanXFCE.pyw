...
class GuessButton:
    def __init__(self, master, letter, xPlace, yPlace):
        self.letter = letter
        self.newButton = tk.Button(master, text=self.letter, width=3, height=2, font='bold', command=lambda: self.clicked(), background="#1c1c1c", foreground="#FFFFFF", bd=1, activebackground="#444444", activeforeground="#EEEEEE")
...
if __name__ == "__main__":
    ...
    # Check if OS is either Linux of Windows in order to serve the proper icon format for tkinter
    if platform == "linux" or platform == "linux2":
        root.iconbitmap("@mainIcon.xbm")
    else:
